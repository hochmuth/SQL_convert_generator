''' SQL Bulk Insert/Convert Generator
    ...
    Parameters:
        ...
        using_various_lengths -- whether we want to set SQL NVARCHAR field lengths based on original SAP values or default 255
...
class DataTypeSearcher:
    ...
    def __init__(self, path, encoding):
        ...
        try:
            ...
            self.dd03l = dd03l_all.loc[:, ['TABNAME', 'FIELDNAME', 'DATATYPE', 'LENG']]
            print('Done')
        except:            
    ...
    def get_field_type(self, file_name, column):
        ...
        if len(join_split) == 1:
            try:
                dtype, dlength = self.dd03l[(self.dd03l.TABNAME == file_name) & (self.dd03l.FIELDNAME == column)][['DATATYPE', 'LENG']].values[0]
                return file_name, file_name, column, dtype, dlength
            except:
                return file_name, file_name, column, dtype, dlength
        ...
        for index, part in enumerate(join_split):                    
            if part in self.all_tables:
                ...
                if index == 0:
                    ...
                    try:
                        dtype, dlength = self.dd03l[(self.dd03l.TABNAME == part) & (self.dd03l.FIELDNAME == field_name)][['DATATYPE', 'LENG']].values[0]
                        return file_name, part, field_name, dtype, dlength
                    except:
                        pass
            ...
            # If the first word doesn't return any match in DD03L
                ...
                if index == 0:                
                    ...
                    if part in self.all_tables:
                        ...
                        try:
                            dtype, dlength = self.dd03l[(self.dd03l.TABNAME == table) & (self.dd03l.FIELDNAME == field_name)][['DATATYPE', 'LENG']].values[0]
                        except:
                            pass
                            # Return the tuple in any case
                        return file_name, table, field_name, dtype, dlength
        ...
        for table_list in self.internal_list:
            ...
            for field_list in table_list:
                ...
                if fin_field == last_field:
                    self.output_file.write('    ['+fin_field+'] NVARCHAR('+fin_dleng+')\n')
                    self.output_file.write(')\n')
